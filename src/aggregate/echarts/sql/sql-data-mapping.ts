...
export function buildSqlMappingExpressions(input: MappingInput): MappingOutput {
    ...
    const mapped = series.map((s) => {
      ...
      if (!key) {
        ...
        if (bucket === 'none') {
          ...
          dataExpr = `(function(){ ${bucketPrelude} var cats = (${xExprSorted}); return cats.map(function(k){ var cnt=0; rows.forEach(function(r){ var xv=r[${JSON.stringify(xKey)}]; if(__bucketKey(xv)===k) cnt++; }); return cnt; }); })()`;
        }
      } else if (agg === 'sum') {
        if (bucket === 'none') {
        ...
        if (bucket === 'none') {
          ...
          dataExpr = `(function(){ ${bucketPrelude} var cats = (${xExprSorted}); function toNum(x){ if(x==null) return null; if(typeof x==='number') return isFinite(x)?x:null; if(typeof x==='boolean') return x?1:0; var s=String(x).trim(); if(!s) return null; s=s.replace(/,/g,''); var m=s.match(/^(-?\\d+(?:\\.\\d+)?)(%)$/); if(m) return parseFloat(m[1])/100; var n=Number(s); return isNaN(n)?null:n; } return cats.map(function(k){ var arr = []; rows.forEach(function(r){ var xv=r[${JSON.stringify(xKey)}]; if(__bucketKey(xv)===k){ var n=toNum(r[${JSON.stringify(key)}]); if(n!=null) arr.push(n); } }); return arr.length? (arr.reduce(function(a,b){return a+b;},0)/arr.length):0; }); })()`;
        }
      } else if (agg === 'min') {
        if (bucket === 'none') {
  ...
  const baseArr = (!xKey)
    ? 'rows.map((_, i) => String(i+1))'
    : (bucket === 'none'
      ? `rows.map(function(r){ var xv = r[${JSON.stringify(xKey)}]; return String(xv); })`
      : `(()=>{ ${bucketFns[bucket as 'year' | 'month' | 'day' | 'hour']} return rows.map(function(r){ var xv=r[${JSON.stringify(xKey)}]; var k=__bucketKey(xv); return k==null?'':k; }); })()`);
