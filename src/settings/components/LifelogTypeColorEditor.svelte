<script lang="ts">
  ...
  export let value: any; // 格式: "类型1=颜色1\n类型2=颜色2"

  const dispatch = createEventDispatcher();

  type TypeColorPair = { type: string; color: string };
  let pairs: TypeColorPair[] = [];
  ...
  // 解析初始值
  function parseInitial(): TypeColorPair[] {
    if (!value) return [];
    const lines = String(value).split(/\r?\n/);
    const result: TypeColorPair[] = [];
  ...
  function emitChange() {
    const lines = pairs
      .filter(p => p.type.trim() && p.color.trim())
      .map(p => `${p.type.trim()}=${p.color.trim()}`);
  ...
  // 添加新行
  function addRow() {
    pairs = [...pairs, { type: '', color: presetColors[pairs.length % presetColors.length] }];
  }
  ...
  function updateType(index: number, newType: string) {
    pairs[index] = { ...pairs[index], type: newType };
    pairs = [...pairs];
    emitChange();
  }
...
<div class="tag-color-map-editor">
  <div class="pairs-container">
    {#each pairs as pair, index}
      <div class="pair-row">
        <input
          class="b3-text-field"
          placeholder="输入类型名称（如 工作/学习/运动）"
          value={pair.type}
          on:input={(e) => updateType(index, e.currentTarget.value)}
        />
  ...
  <div class="preview-section">
    ...
    <div class="preview-tags">
      {#each pairs.filter(p => p.type && p.color) as pair}
        <span
          class="preview-tag"
...
<script context="module" lang="ts">
  ...
  export function guessTextColor(bgColor: string): string {
