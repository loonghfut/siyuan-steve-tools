export function getTldrawAgentCapabilities() {
    return {
        ...
        agentPrompt: [
            ...
            'For complex branch edits, inspect/read shape details first. Branch summaries expose rootShapeId, leftChildIds, and rightChildIds; use those ids instead of guessing from visual x/y positions.',
        ...
        toolSelectionHints: {
            shapeCommand: {
                ...
                examples: [
                    ...
                    { intent: 'createShapes', node: { kind: 'js-shape', w: 360, h: 240, interactive: true, script: "const { dom, getData, saveData } = api; const render = () => { const state = getData() || { count: 0 }; dom.innerHTML = '<button>Count: ' + state.count + '</button>'; dom.querySelector('button')?.addEventListener('click', (event) => { event.stopPropagation(); saveData({ count: state.count + 1 }); render(); }); }; render();" } },
					{ intent: 'updateShape', target: '$selection[0]', patch: { script: '/* complete corrected JS source */' } },
        ...
        customShapeGuide: [
            ...
            },
            {
                ...
                notes: [
                    ...
                    'Syntax errors are returned before creation. Runtime errors are returned in create/update results and included as safe runtime diagnostics in shape details; use updateShape with a complete patch.script on the returned/selected original shape to correct them. Never retry by creating another js-shape: the command blocks that duplicate-creation loop while the failed shape remains selected. Current source remains write-only and raw data stays protected.',
