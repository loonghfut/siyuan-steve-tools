...
export function createCreateBasicShapeAction(): AgentActionDefinition {
    return {
        ...
        description: 'Create a safe built-in/custom tldraw shape on an open whiteboard. Required args: whiteboardId, kind "text"|"note"|"geo"|"arrow"|"line"|"draw"|"highlight"|"frame"|"bezier-connector"|"slide"|"mind-map"|"js-shape". All other props have defaults; only pass optional overrides you need: x, y, w, h, color, text, geo, name, direction, theme, select, zoom default true. Line is an arrow without arrowheads. JS shape uses a restricted placeholder; script content is not accepted. Unknown args are rejected.',
        handler: async (args) => {
