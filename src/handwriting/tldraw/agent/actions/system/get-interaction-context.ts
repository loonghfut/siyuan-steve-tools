...
export function createGetInteractionContextAction(): AgentActionDefinition {
    return {
        ...
        description: 'Sense compact current STtools tldraw user context before using whiteboard tools. Returns focusedWhiteboardId, all open whiteboards, whether the user has selected shapes, and selectedShapeCount. Call this first before any tldraw create/edit/navigate/read operation, then use focusedWhiteboardId as whiteboardId unless the user asked for a different board. Optional args: includeSelectedShapeIds boolean default false for low-level tools, includeSelectedShapeDetails boolean default false, selectedShapeLimit number default 5. Selected card/single-block details include concrete linked SiYuan block content.',
        handler: async (args) => {
