...
export function createCreateBasicShapeAction(): AgentActionDefinition {
    return {
        ...
        description: 'Create a safe built-in/custom tldraw shape on an open whiteboard. Required args: whiteboardId, kind "text"|"note"|"geo"|"arrow"|"line"|"draw"|"highlight"|"frame"|"bezier-connector"|"slide"|"mind-map"|"js-shape". Default to text for unlinked annotations and frame for grouping/boundaries; for relationships use tldraw_create_connector, whose default is bezier-connector. Avoid note, geo, arrow, line, draw, highlight, slide, mind-map, and js-shape unless the user explicitly requests them or the task clearly requires that specific shape. Use tldraw_create_shape only for semantic card/single-block/branch. Common optional args: x, y, w, h, color, text, geo, name, blockId, direction, theme, select, zoom default true. Line is an arrow without arrowheads. For connecting two existing shapes, prefer tldraw_create_connector so ports/bindings are created. JS shape script content is not accepted. Unknown args are rejected.',
        handler: async (args) => {
