...
export function createCreateConnectorAction(): AgentActionDefinition {
    return {
        ...
        description: 'Create a safe connector on an open whiteboard. Required args: whiteboardId and either shapeIds [sourceId,targetId], startShapeId/endShapeId, or start/end points. Optional args: kind "arrow"|"bezier-connector" default "bezier-connector", color, text, strokeWidth, select, zoom default true, resultMode "compact"|"full" default compact. Prefer shapeIds for ordinary relationship lines between existing shapes; ports are chosen automatically for custom card/single-block/mind-map ports when available. For mind maps, tree/parent-child/hierarchical layouts, prefer tldraw_apply_plan op "branch". Use bezier-connector for durable labeled relationships, arrow for simpler tldraw arrows, and plain line only for non-binding visual marks. Unknown args are rejected.',
        handler: async (args) => {
