...
export function createCreateShapeAction(): AgentActionDefinition {
    return {
        ...
        description: 'Create an STtools semantic/business shape on an open whiteboard. Required args: whiteboardId, kind "card"|"single-block"|"branch". Default to card for document/heading/substantial content and single-block for compact paragraph-like items. Card has two simple modes: pass blockId to reference an existing document/heading block, or pass contentMarkdown with optional title to create a new heading block and insert the content after it; do not pass both blockId and contentMarkdown. If neither is passed for card, an empty heading block is created and bound automatically. Entity shapes auto-avoid overlap and return final bounds. Avoid branch unless explicitly requested or clearly required. For text/frame/bezier-connector and other basic kinds, prefer tldraw_create_basic_shape or tldraw_create_connector; this action redirects basic kinds only for compatibility. Common optional args: x, y, w, h, color, blockId, contentMarkdown, title, select, zoom default true. If blockId is provided, card blockId must be a SiYuan document or heading block id, and single-block blockId must be a paragraph block id. Card-only optional: isMain, isCollapsed, showMask. Branch optional: rootShapeId, childIds, children, leftChildren, rightChildren, direction, horizontalGap, verticalGap, lineStyle "curve-solid"|"elbow-solid"|"straight-solid"|"curve-dashed"|"frame-floating", lineWidth, snapDistance, showBackground. Branch children may be existing shape IDs or objects like {shapeId} / {kind:"card"|"single-block", blockId|contentMarkdown, title, side}. Unknown args are rejected.',
        handler: async (args) => {
