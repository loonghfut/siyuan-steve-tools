...
- feat: 添加手动清理未使用资源功能，避免初始化时误删资产 (loonghfut)
- feat: add JS Shape functionality with editor integration (loonghfut)
- feat: 升级 JS 形状脚本编辑器，集成 CodeMirror 6，支持语法高亮和代码补全 (loonghfut)
- feat: 添加高对比度主题支持及主题切换功能，提升可访问性 (loonghfut)
- feat: 升级 CodeMirror 编辑器，添加撤销、重做和格式化功能，优化主题切换体验 (loonghfut)
- feat: 更新 JS 形状类型和属性，添加数据持久化支持 (loonghfut)
- feat: 更新 JS 形状工具，添加清空已保存数据功能，优化数据展示 (loonghfut)
- feat: 添加主题观察者和主题应用功能，优化编辑器主题适配 (loonghfut)
- feat: 重构 JS 形状工具，提取静态内容并优化脚本编辑器 UI (loonghfut)
- feat: 优化 JS 形状编辑器 UI，调整高度并更新 API 文档描述 (loonghfut)
- feat: 为运行容器添加 click 兼容，优化交互体验 (loonghfut)
- feat: 更新默认脚本，添加形状信息支持，优化交互提示 (loonghfut)
- feat: 增强 CodeEditor 补全功能，支持环境变量和形状属性的自动补全 (loonghfut)
- feat: 为 CodeEditor 添加运行时错误注释功能，支持行列信息标记 (loonghfut)
- fix: 修复 OverlayShape 类型检查和 Toolbar 中 js-shape 项目的顺序 (loonghfut)
- feat: 添加 SVG 导出功能，支持所见即所得的运行时 DOM 序列化 (loonghfut)
- chore: 删除过时的 JS 形状运行时 API 文档 (loonghfut)
- feat: 添加 JS 形状脚本执行限制功能，支持全局禁用和容器内 DOM 访问限制 (loonghfut)
- fix: 优化事件处理，确保在编辑和重新运行 JS 形状时正确注册和注销事件监听器 (loonghfut)
- fix: 移除自动运行功能，简化样式面板中的交互按钮布局 (loonghfut)
