...
为方便和打赏用户沟通，故临时建了反馈QQ群，加之前请填一下[申请表](https://www.kdocs.cn/wo/sl/v1lC0R0)
...
### v0.12.7 (2025年08月20日)
- feat: 添加 WPS 数据处理功能，支持缩放比例和新数据服务模块 (loonghfut)
- fix: 优化 iframe 和 webview dock 更新逻辑，确保缩放比例正确应用 (loonghfut)
- feat: WPS链接插入，dock嵌入手机端页面 (loonghfut)
- feat: 添加 WPS 文件管理功能，支持文件链接和自定义按钮 (loonghfut)
- feat: 添加获取光标块ID功能，并在WPS文件服务中集成插入逻辑 (loonghfut)
- fix: 优化 webview 鼠标移动监听逻辑，增强兼容性 (loonghfut)
- feat: 添加根据 URL 生成链接卡片的功能，并在 WPS 数据服务中集成 (loonghfut)
- 取消拦截 (loonghfut)
- fix: 移除未使用的拦截链接卡片点击的导入 (loonghfut)
- feat: 更新生成链接卡片功能，支持自定义按钮和点击事件 (loonghfut)
- feat: 集成生成链接卡片功能到 WpsFileServ，优化插入逻辑 (loonghfut)
- feat: 优化链接卡片按钮样式，增强交互效果 (loonghfut)
- feat: 优化链接卡片按钮样式，调整按钮宽度计算和内边距逻辑 (loonghfut)
- feat: 添加 ChangeLinkStyle 函数并在 WpsFileServ 中集成，支持链接样式变更 (loonghfut)
- feat: 在插入的链接卡片中添加自定义属性，支持更灵活的样式调整 (loonghfut)
- feat:实现预览视图转卡片 (loonghfut)
- 完善细节 (loonghfut)
- feat: 在 WpsFileServ 中集成 ShowLinkContent 函数，支持链接内容预览 (loonghfut)
- 优化预览细节 (loonghfut)
- 兼容链接转换卡片 (loonghfut)
- fix: 更新 WPS 文件图标为云图标 (loonghfut)
- feat: 添加 WPS 文件图标并更新初始化逻辑 (loonghfut)
- fix: 更新 WPS文件管理的默认文件夹链接 (loonghfut)
- 优化细节 (loonghfut)
- fix: 更新插件版本至 0.12.6 (loonghfut)
- fix: 更新插件版本至 0.12.7，并恢复模块配置 (loonghfut)
