...
### v0.44.0 (2026年06月26日)
- feat(branch-shape): 添加线宽、间距和吸附距离的样式控制滑块 (loonghfut)
- refactor(branch-style): 移除吸附距离状态计算逻辑 (loonghfut)
- feat(branch-frame): 实现自动外框增强样式与布局逻辑 (loonghfut)
- fix(branch-layout): 修复拖拽后关联分支未重新布局的问题 (loonghfut)
- fix(branch-layout): 修复删除形状后关联分支未重新布局的问题 (loonghfut)
- perf(branch-layout): 预计算形状边界与页面形状列表，优化吸附和拖拽布局性能 (loonghfut)
- refactor(branch-style): 将外框样式重构为半透明背景填充，移除描边与虚线 (loonghfut)
- feat(branch-shape): 将 showOuterFrame 字段重命名为 showBackground，添加版本 3 数据迁移 (loonghfut)
- chore(plugin): 更新版本号至 0.44.0 (loonghfut)
