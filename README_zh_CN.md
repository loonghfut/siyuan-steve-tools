...
### v0.45.1 (2026年06月27日)
- fix(branch): 修复创建分支时子元素引用不一致的问题 (loonghfut)
- `feat(branch): 添加分支子元素布局推断机制与素材库自动布局支持` (loonghfut)
- fix(branch): 修复分支拖拽时未校验连接状态的问题 (loonghfut)
- feat(branch): 实现分支单块创建功能与 UI 交互支持 (loonghfut)
- `feat(branch-style): 新增分支线条样式（曲线/折线/直线/虚线）配置与渲染支持` (loonghfut)
- `feat(icon): 添加分支线条样式 SVG 图标资源` (loonghfut)
- `feat(branch-style): 新增 frame-floating 浮动框分支样式支持` (loonghfut)
- fix(branch-style): 修复浮动样式下自动外框显示逻辑并优化背景切换重排行为 (loonghfut)
- perf(branch): 优化分支重排防抖逻辑并实现分支级交互状态订阅 (loonghfut)
- feat(icon): 新增 30+ 自定义 SVG 图标并统一替换样式面板文字标识 (loonghfut)
- style(style-section): 统一样式面板图标尺寸，移除 TldrawUiIcon small 属性 (loonghfut)
- feat(icon): 新增左右分支添加 SVG 图标并替换 InFrontOfCanvas 文字标识 (loonghfut)
- refactor(icon): 将自定义图标配置从 tldraw-manager 提取至独立工具文件 (loonghfut)
- refactor(card-collapse): 提取折叠高度常量，消除硬编码阈值 (loonghfut)
- fix(card): 修复折叠状态文本溢出问题，添加行数限制与省略号截断 (loonghfut)
- chore(plugin): 更新版本号至 0.45.0 并精简中文描述 (loonghfut)
- perf(card): 移除卡片高度过渡动画与冗余 transition 属性，降低渲染开销 (loonghfut)
- chore(version): 升级版本号至 0.45.1 (loonghfut)
