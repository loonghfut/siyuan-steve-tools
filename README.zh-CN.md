...
### v0.51.0 (2026年07月22日)
- feat(icon): 统一升级自定义图标为 lucide 24x24 标准规格 (loonghfut)
- refactor(style-panel): 统一样式面板按钮样式，移除内联样式 (loonghfut)
- feat(card): 卡片内容编辑按钮改为弹窗模式 (loonghfut)
- fix(handwriting): 修复 tldraw 工具栏 extras 控件背景色与主题不一致的问题 (loonghfut)
- refactor(card-shape): 重构卡片内容高度测量逻辑，修复短卡片高度计算异常问题 (loonghfut)
- feat(handwriting): 为 tldraw agent 添加 js-shape 自定义形状支持，扩展类型定义与验证逻辑 (loonghfut)
- feat(agent): 增强 js-shape 执行反馈机制，支持运行时状态自动上报与错误修正 (loonghfut)
- fix(agent): 防止 js-shape 运行时错误导致重复创建，强制修正原形状 (loonghfut)
- chore(plugin): 升级版本号至 0.51.0 (loonghfut)
