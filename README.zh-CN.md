...
- feat(handwriting): 重构连接线扩展菜单为图标按钮模式，移除取消选项并优化工具栏自定义图标 (loonghfut)
- feat(CardShape): 新增点击末尾阻止插入空块配置，优化折叠卡片内容展示 (loonghfut)
- feat(handwriting): 支持引用块作为卡片关联块，优化拖拽数据识别与关联逻辑 (loonghfut)
- feat(handwriting): 新增卡片默认块类型配置项，重构创建逻辑支持标题块与引述块 (loonghfut)
- feat(CardShape): 新增卡片自动收缩至内容高度功能，实现高度计算工具函数并添加样式面板操作按钮 (loonghfut)
- feat(SlideShape): 为幻灯片截图功能添加进度反馈覆盖层，优化关联块更新与上传流程 (loonghfut)
- fix(handwriting): 优化引述块文字颜色，使其更接近正文 (loonghfut)
- feat(handwriting): 双击画布创建功能新增卡片类型支持 (loonghfut)
- feat(BranchShape): 重构连线路径为圆角肘形，优化曲线控制点计算并移除硬编码 midX (loonghfut)
- feat(handwriting): 新增 tree-table 分支连线样式，配套表格布局算法、样式面板选项及嵌套分支重排优化 (loonghfut)
- refactor(BranchShape): 重构 tree-table 布局与命中检测逻辑，新增轮廓及行分隔线辅助函数，修正行分隔线边界并优化交互状态描边显示 (loonghfut)
- fix(BezierConnectorShape): 修正端口选中检测逻辑，仅在形状为唯一选中项时显示覆盖层 (loonghfut)
